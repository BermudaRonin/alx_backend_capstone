...
export default function RegisterForm() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        startLoading();
        if (!value.isMatching('password', 'password2')) return onError("Passwords do not match");
        const response = await actions.registerUser(value.get('username', 'password'));
    ...
    return <form className={classNames.form.root} onReset={handleReset} onSubmit={handleSubmit}>
        ...
        <FormInputRow label="Repeat password" name="password2" value={value} />
